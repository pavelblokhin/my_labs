...
class WordAligner(BaseAligner):
    ...
    def _e_step(self, parallel_corpus: List[TokenizedSentencePair]) -> List[np.array]:
        ...
        for sentence_pair in parallel_corpus:
            ...
            translation_matrix = self.translation_probs[source_tokens, :][:, target_tokens]
            normalization_factors = translation_matrix.sum(axis=0)
            posterior = translation_matrix / normalization_factors
    ...
    def _compute_elbo(self, parallel_corpus: List[TokenizedSentencePair], posteriors: List[np.array]) -> float:
        ...
        """
        elbo = 0
        for parallel_sentence, posterior in zip(parallel_corpus, posteriors):
            source_tokens = parallel_sentence.source_tokens
            target_tokens = parallel_sentence.target_tokens
            norm_constant = np.log(1 / len(source_tokens) + 3.4028235e-30)
            translation_probs = self.translation_probs[source_tokens.reshape(-1, 1), target_tokens]
            elbo_contribution = np.sum(posterior * (np.log(translation_probs) + norm_constant) - posterior * np.log(posterior + 3.4028235e-40))
    ...
    def _m_step(self, parallel_corpus: List[TokenizedSentencePair], posteriors: List[np.array]):
        ...
        for sentence_pair, posterior in zip(parallel_corpus, posteriors):
            np.add.at(self.translation_probs,(sentence_pair.source_tokens[:, None], sentence_pair.target_tokens), posterior)
    ...
    def align(self, sentences):
        ...
        for i, sentence_pair in enumerate(sentences):
            source_pos = posteriors[i].argmax(axis=0) + 1
            target_pos = np.arange(1, posteriors[i].shape[1] + 1)
            alignment = [(s, t) for s, t in zip(source_pos, target_pos)]
